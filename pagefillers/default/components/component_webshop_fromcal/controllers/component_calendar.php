<?php
    ...
    Class Component_calendar_Controller extends Login_Controller {
        ...
        public function add($fieldid) {
            ...
            if (Wi3::$rights->check("edit", $field->page)) {
                ...
                foreach($_POST as $key => $val) {
                    //if the date is sent along, convert it to sortable format
                    //date is in 22-08-2010 format, and should be 20100822
                    if ($key == "date") {
                        $sortabledate = substr($val, 6,4) . substr($val, 3,2) . substr($val, 0, 2);
                        $item->sortabledate = $sortabledate;
                    }
                    $item->$key = $val;
                }
                ...
                //we here use the ->title to create the slug for an item. One could also use the unique arrayname, which is generated when saving an sqlarray
                // --------------------------------------------------
                if (isset($item->title)) {
                    $slug= url::title($item->title);
                    ...
                    "slug" => $slug
                        ...
                        $counter++;
                        $slug= url::title($item->title . "-" . $counter);
                    }
                } else {
