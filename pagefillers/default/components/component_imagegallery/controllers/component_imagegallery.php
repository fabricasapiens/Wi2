<?php
    ...
    Class Component_imagegallery_Controller extends Login_Controller {
        ...
        function selectfolder($fieldid) {
            ...
            if (Wi3::$rights->check("edit", $field->page)) {
                $field->set("folderid", $_POST["folderid"]);
                echo str_replace("\\n", "", json_encode(Array(
                    "alert" => "map succesvol ingesteld",
                    "scriptsbefore" => array("wi3.pagefiller.editing.reloadCurrentField();"),
                    "scriptsafter" => array("$('#component_imagegallery_edit_tabs-1 li').css('font-weight', 'normal').filter('#component_imagegallery_edit_li_" . $_POST["folderid"] . "').css('font-weight', 'bold');"),
                )));
            }
        }
